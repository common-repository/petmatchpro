...
<?php
	//echo '<pre>Detail Template Called with Details<br>'; print_r($animalDetails); echo '</pre>';
	...
	$photoLevelKey = constant('LEVEL_PREFIX_ANIMAL_DETAIL') . constant('ANIMALSFIRST_PHOTOS') . '_' . constant('ADOPT_METHODTYPE_ANIMALSFIRST');
  	if ( ($this->PMPLicenseTypeID <= $pmpFieldLevelsAdopt[$photoLevelKey]) ) {
  		...
  		//echo '<pre>Photo Array<br>' . print_r($photoArray) . '</pre>';
	...
	//echo 'Social Share Level = ' . $pmpFieldLevelsAdopt['level_social_share_adopt'] . '<br>';
	$displaySocial = 0;
	...
	$displayPoster = 0;
 	if ( ($this->PMPLicenseTypeID <= constant('FREE_LEVEL')) && (!is_null($posterURL)) ) {
    	...
    	$gaParamArray['event_action'] = 'Select';
   		$clickID = 'pmp-detail-print-poster-' . constant('ADOPT_METHODTYPE_ANIMALSFIRST') . '-' . $animalSpeciesLower . '-' . str_replace(" ", "-", $animalNameLower);
   		$gaParamArray['click_id'] = $clickID;
   		...
   		$gaParamArray['click_url'] = $clickURL;
   		$onClick = $this->allAPIFunction->onClickValue($gaName, $gaParamArray);  
    	$output .= '	<div class="pmp-details-features"><a id="' . $clickID . '" target="_blank" class="pmp-button pmp-detail-print-poster-button" href="' . $clickURL . '?method=' . $callFunc . '&' . constant('ANIMALSFIRST_ID'). '='. $_GET[constant('ANIMALSFIRST_ID')] . '" title="Click to Print ' . $animalName . "'s" . ' Poster" onclick="' . $onClick . '">' . $clickText . '</a>';
    	$output .= '</div> <!-- .pmp-details-features-->';
    	$displayPoster = 1;
	}
    ...
    //Add Email Us Button
    if ( ($adopt_email != '') && ($location_value == $shelter) ) {
    ...
    } elseif ( ($foster_email != '') && ($location_value == $foster_home) ) {
		$output .= '<div class="pmp-detail-conversion-button pmp-detail-email-us">'; 
		if ($this->PMPLicenseTypeID <= $pmpFieldLevelsAdopt[$conversionButtonLevelKey]) {
    ...
    //Add Call Us Button
    if ( ($foster_phone != '') && ($location_value == $foster_home) ) {
