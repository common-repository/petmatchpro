...
<?php
	...
	$output .= '<h2 id="pmp-details-title-' . constant('FOUND_METHODTYPE_PETPOINT') . '" class="pmp-details-title">' .$animalDetails[constant('PETPOINT_SEX')]['value'] . ' ' . $animalSpecies . ' Found in ' . $animalDetails[constant('PETPOINT_JURISDICTION')]['value'] . '</h2>';  	
  	...
  	$output .= '<div id="pmp-details-wrapper-' . constant('FOUND_METHODTYPE_PETPOINT') . '" class="pmp-details-wrapper">';  	$output .= '  <div class="pmp-details-media">';
	...
	$found_phone = $contact_options[constant('FOUND_METHODTYPE_PETPOINT') . '_phone'];  

	$foundEmail = $contact_options[constant('FOUND_METHODTYPE_PETPOINT') . '_email'];
    ...
    if ( (!is_null($foundEmail)) ) {
		...
		if ($this->PMPLicenseTypeID <= $pmpFieldLevelsFound['level_buttons_conversion_' . constant('FOUND_METHODTYPE_PETPOINT')]) {
    		...
    		$clickURL = 'mailto:' . $foundEmail . '?subject=IMPORTANT: Found Inquiry for ' . $animalID . ' the ' . $animalSpecies;
