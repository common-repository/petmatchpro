...
//	  	if ( $this->PMPLicenseTypeID <= $pmpFieldLevelsLost['level_detail_photo_lost'] ) {		
	...
	$displayPoster = 0;
 	...
 	if ( ($this->PMPLicenseTypeID <= constant('FREE_LEVEL')) && (!is_null($posterURL)) ) {
    	...
    	$gaParamArray['event_action'] = 'Select';
   		...
   		$clickID = 'pmp-detail-print-poster-' . constant('LOST_METHODTYPE_PETPOINT') . '-' . $animalSpeciesLower . '-' . $animalID;
    ...
    foreach($showItems as $detailKey){
   		...
   		if(array_key_exists($detailKey, $animalDetails)) {
    		...
    		$levelKey = constant('LEVEL_PREFIX_ANIMAL_DETAIL') . $detailKey . '_lost';   
