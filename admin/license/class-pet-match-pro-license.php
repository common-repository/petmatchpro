<?php
if(!class_exists("PetMatchProBase")) {
	class PetMatchProBase {
    	public $key = "947087BAE8B6743C";
    	private $product_id = "1";
    	private $product_base = "pet-match-pro";
    	private $server_host = "https://petmatchpro.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='') {
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_pet-match-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=pet-match-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
            ...
            if(function_exists("wp_remote_get")) {
                ...
                if ( is_array( $response ) ) {
                    ...
                    if ( is_object( $responseJson ) && ! empty( $responseJson->status ) && ! empty( $responseJson->data->new_version ) ) {
                        ...
                        $responseJson->data->icons['default']  = plugins_url('assets/icon-256x256.png',$this->pluginFile); /* Added by Appdbd */
                        ...
                        $responseJson->data->banners_rtl 	= (array) $responseJson->data->banners;						
                    ...
                    if ( ! empty( $response->status ) ) {
                        if ( ! empty( $response->data ) ) {
                            ...
                            if ( $licenseObj->is_valid ) {
								...
								$responseObj->lic_param            = !empty($licenseObj->lic_param)?$licenseObj->lic_param:null; /*RMB - To Report License Type ID for License Management */
