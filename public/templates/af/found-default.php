...
<?php
    //echo '<pre>FOUND DEFAULT Template Called with Details<br>'; print_r($animalDetails); echo '</pre>';
	...
	$output .= '<h2 id="pmp-details-title-' . constant('FOUND_METHODTYPE_ANIMALSFIRST') . '" class="pmp-details-title">' .$animalDetails[constant('ANIMALSFIRST_GENDER')]['value'] . ' ' . $animalSpecies . ' ' . ucfirst(constant('FOUND_METHODTYPE_ANIMALSFIRST')) . ' in ' . $foundCity . '</h2>';  	
  	$output .= '<div id="pmp-details-wrapper-' . constant('FOUND_METHODTYPE_ANIMALSFIRST') . '" class="pmp-details-wrapper">';  	$output .= '  <div class="pmp-details-media">';
	...
	if (array_key_exists(constant('FOUND_METHODTYPE_ANIMALSFIRST') . '_email', $this->contactOptions)) {
		$foundEmail = $contact_options[constant('FOUND_METHODTYPE_ANIMALSFIRST') . '_email'];
	} else {
		$foundEmail = constant('EMPTY_VALUE');	
	}
    ...
    if ( (!is_null($foundEmail)) ) {
		...
		if ($this->PMPLicenseTypeID <= $pmpFieldLevelsFound[$conversionButtonLevelKey]) {
	   		...
	   		$gaParamArray['click_text'] = $clickText;    	    
    		$clickURL = 'mailto:' . $foundEmail . '?subject=IMPORTANT: ' . ucfirst(constant('FOUND_METHODTYPE_ANIMALSFIRST')) . ' Inquiry for ' . $animalID . ' the ' . $animalSpecies;
    		$gaParamArray['click_url'] = $clickURL;
    		$onClick = $this->allAPIFunction->onClickValue($gaName, $gaParamArray);  	
