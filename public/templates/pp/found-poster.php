...
<?php
    ...
    $levelsFile = 'pmp-field-levels-' . constant('FOUND_METHODTYPE_PETPOINT') . '.php';
	...
	$output .= '<h2 id="pmp-details-title-' . constant('FOUND_METHODTYPE_PETPOINT') . '" class="pmp-details-title">' .$animalDetails[constant('PETPOINT_SEX')]['value'] . ' ' . $animalSpecies . ' Found in ' . $animalDetails[constant('PETPOINT_JURISDICTION')]['value'] . '</h2>';  	

  	$output .= '<div id="pmp-details-wrapper-' . constant('FOUND_METHODTYPE_PETPOINT') . '" class="pmp-details-wrapper">';

  	$output .= '  <div class="pmp-details-media">';

	$output .= '    <div class="pmp-detail-image">'; 

  	if ( (array_key_exists(constant('PETPOINT_PHOTO'), $animalDetails)) && ($this->PMPLicenseTypeID <= $pmpFieldLevelsFound['level_detail_' . constant('PETPOINT_PHOTO') . '_' . constant('FOUND_METHODTYPE_PETPOINT')]) ) {
    ...
    if ( !empty($this->PMPLicenseTypeID) && ($this->PMPLicenseTypeID != constant('FREE_LEVEL')) ) {
		...
		$thumb = 0;		
	  	...
	  	if ( (array_key_exists(constant('PETPOINT_PHOTO'), $animalDetails)) && ($this->PMPLicenseTypeID <= $pmpFieldLevelsFound['level_detail_' . constant('PETPOINT_PHOTO') . '_' . constant('FOUND_METHODTYPE_PETPOINT')]) && ($thumb <= $maxThumbs) ) {	

	    	foreach($animalDetails[constant('PETPOINT_PHOTO')] as $photoURL){

			   	$clickID = 'pmp-detail-thumb-' . constant('FOUND_METHODTYPE_PETPOINT'). '-' . $animalSpeciesLower . '-' . $animalID . '-' . $thumb;

			   	$gaParamArray['click_id'] = $clickID;

			   	$clickText = 'View Found ' . $animalSpecies . ' Photo #' . $thumb;
	...
	$displaySocial = 0;	
 	...
 	if ( ($this->PMPLicenseTypeID <= $pmpFieldLevelsFound['level_social_share_' . constant('FOUND_METHODTYPE_PETPOINT')]) && (array_key_exists('social_share', $this->contactOptions)) ) {		
	...
	$displayPoster = 0;
 	...
 	if ( ($this->PMPLicenseTypeID <= constant('FREE_LEVEL')) && (!is_null($posterURL)) ) {
    	...
    	$gaParamArray['event_action'] = 'Select';
   		...
   		$clickID = 'pmp-detail-print-poster-' . constant('FOUND_METHODTYPE_PETPOINT') . '-' . $animalSpeciesLower . '-' . $animalID;
    	...
    	$output .= '	<div class="pmp-details-features"><a id="' . $clickID . '" target="_blank" class="pmp-button pmp-detail-print-poster-button" href="' . $clickURL . '?method=' . $callFunc . '&animalID='. $_GET['animalID'] . '" title="Click to Print Poster" onclick="' . $onClick . '">' . $clickText . '</a>';
    ...
    $output .= '  </div> <!-- .pmp-details-media -->';
   	...
   	$output .= '  <div id="pmp-details-results-wrapper-' . constant('FOUND_METHODTYPE_PETPOINT') . '" class="pmp-details-results-wrapper">'; 
    ...
    foreach($showItems as $detailKey){
   		...
   		if(array_key_exists($detailKey, $animalDetails)) {
    		...
    		$levelKey = constant('LEVEL_PREFIX_ANIMAL_DETAIL') . $detailKey . '_' . constant('FOUND_METHODTYPE_PETPOINT');   
	...
	$output .= '<div id="pmp-details-other-buttons-' . constant('FOUND_METHODTYPE_PETPOINT') . '" class="pmp-details-other-buttons">';

	

   	//Add Button to Call Shelter

	$found_phone = $this->contactOptions[constant('FOUND_METHODTYPE_PETPOINT') . '_phone'];  



   	if ( !is_null($found_phone) ) {

   		$output .= '    <div class="pmp-detail-other-button pmp-detail-call-us">';       	

    	$clickID = 'pmp-detail-call-us-' . constant('FOUND_METHODTYPE_PETPOINT') . '-' . $animalSpeciesLower . '-' . $animalID;

		if ($this->PMPLicenseTypeID <= $pmpFieldLevelsFound['level_buttons_conversion_' . constant('FOUND_METHODTYPE_PETPOINT')]) {  
    ...
    if ($referer == constant('REFERER_SEARCH')) {
    	...
    	$clickID = 'pmp-detail-search-return-' . constant('FOUND_METHODTYPE_PETPOINT') . '-' . $animalSpeciesLower . '-' . $animalID;
    ...
    $output .= '    </div> <!-- #pmp-details-other-buttons-' . constant('FOUND_METHODTYPE_PETPOINT') . ' -->';         

    $output .= '  </div> <!-- #pmp-details-results-wrapper-' . constant('FOUND_METHODTYPE_PETPOINT') . ' -->';

    $output .= '</div> <!-- #pmp-details-wrapper-' . constant('FOUND_METHODTYPE_PETPOINT') . ' -->';
