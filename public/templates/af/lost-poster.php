...
<?php
    //echo '<pre>LOST POSTER Template Called with Details<br>'; print_r($animalDetails); echo '</pre>';
	...
	$displayPoster = 0;
 	if ( ($this->PMPLicenseTypeID <= constant('FREE_LEVEL')) && (!is_null($posterURL)) ) {
    	...
    	$gaParamArray['event_action'] = 'Select';
   		$clickID = 'pmp-detail-print-poster-' . constant('LOST_METHODTYPE_ANIMALSFIRST') . '-' . $animalSpeciesLower . '-' . $animalID;
   		$gaParamArray['click_id'] = $clickID;
