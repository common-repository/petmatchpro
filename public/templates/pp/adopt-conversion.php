...
<?php
	...
	if ( !$adopt_link ) {
   	...
   	if ( !is_null($adopt_link) ) {   	
   		...
   		$output .= '    <div class="pmp-detail-other-button pmp-detail-application-' . constant('ADOPT_METHODTYPE_PETPOINT') . '">';
		...
		if ($this->PMPLicenseTypeID <= $pmpFieldLevelsAdopt['level_buttons_conversion_' . constant('ADOPT_METHODTYPE_PETPOINT')]) {  
	    	...
	    	$output .= '      <a id="' . $clickID . '" target="' . $adopt_link_target . '" class="pmp-button pmp-detail-application-' . constant('ADOPT_METHODTYPE_PETPOINT') . '-button" href="' . $clickURL . '" title="Complete Adoption Application" onclick="' . $onClick . '">' . $clickText . '</a>';
