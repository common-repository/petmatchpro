...
<?php
    ...
    } elseif ( ($adopt_link != '') && ($location_value == $foster_home) ) {
		$output .= '<div class="pmp-detail-conversion-button pmp-detail-ask-about">'; 
		if ($this->PMPLicenseTypeID <= $pmpFieldLevelsAdopt[$conversionButtonLevelKey]) {
		   	$clickID = 'pmp-detail-ask-about-' . constant('ADOPT_METHODTYPE_ANIMALSFIRST') . '-' . $animalSpeciesLower . '-' . str_replace(" ", "-", $animalNameLower); 
		   	$gaParamArray['click_id'] = $clickID;       	   				
		   	$clickText = 'Ask Us About ' . $animalName;
		   	$gaParamArray['click_text'] = $clickText; 
	       	/* $clickURL = 'mailto:' . $foster_email . '?subject=IMPORTANT: Ask Us About Inquiry for ' . $animalName . ' the ' . $animalSpecies; - Removed Nov 2023 */
	       	$clickURL = $adopt_link;
	    	$gaParamArray['click_url'] = $clickURL;
	    	$onClick = $this->allAPIFunction->onClickValue($gaName, $gaParamArray);
